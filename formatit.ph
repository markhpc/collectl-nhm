...
sub buildSubsysHash
{
  ...
  $hash->{Z} = 1 if ($subsys=~/Z/);
  $hash->{yi} = 1 if ($subsys=~/y/i);
  $hash->{bi} = 1 if ($subsys=~/b/i);
  $hash->{cdi} = 1 if ($subsys=~/c|d/i);
  $hash->{ji} = 1 if ($subsys=~/j/i);
  $hash->{li} = 1 if ($subsys=~/l/i);
  $hash->{l} = 1 if ($subsys=~/l/);
  $hash->{c} = 1 if ($subsys=~/c/);
  $hash->{E} = 1 if ($subsys=~/E/);
  $hash->{di} = 1 if ($subsys=~/d/i);
  $hash->{i} = 1 if ($subsys=~/i/);
  $hash->{fi} = 1 if ($subsys=~/f/i);
  $hash->{m} = 1 if ($subsys=~/m/);
  $hash->{s} = 1 if ($subsys=~/s/);
  $hash->{ni} = 1 if ($subsys=~/n/i);
  $hash->{ti} = 1 if ($subsys=~/t/i);
  $hash->{x} = 1 if ($subsys=~/x/);

  return $hash; 
}

sub buildSubsysArray
{
  my $subsys = shift;
  my $array = [];
  my $present = 0;
  ...
  # Check for everything we didn't explicitly look for in the main loop
  # (basically everything but processes, slabs, and interrupt data). 
  if ($subsys=~/b/i)
  {
    push(@$array, ['buddy', 0, 5, \&dataAnalyzeBuddy]);
  }
  if ($subsys=~/c|d/i)
  {
     push(@$array, ['cpu', 0, 3, \&dataAnalyzeCPU]);

  }
  if ($subsys=~/l/i)
  {
    push(@$array, ['OST_', 0, 4, \&dataAnalyzeLustre]);
    push(@$array, ['OST-b_', 0, 6, \&dataAnalyzeLustreBuffer]);
    push(@$array, ['LLITE:', 0, 6, \&dataAnalyzeLustreLite]);
    push(@$array, ['LLITE_RA:', 0, 9, \&dataAnalyzeLustreLiteRA]);
    push(@$array, ['LLITE_RPC:', 0, 10, \&dataAnalyzeLustreLiteRPC]);
    push(@$array, ['LLDET:', 0, 6, \&dataAnalyzeLustreLDET]);
    push(@$array, ['LUS-d_', 0, 6, \&dataAnalyzeLustreDisk]);

  }
  if ($subsys=~/l/)
  {
    push(@$array, ['MDS', 0, 3, \&dataAnalyzeLustreMDS]);
  }
  if ($subsys=~/c/)
  {
     push(@$array, ['intr', 0, 4, \&dataAnalyzeCPUInterrupt]);
     push(@$array, ['ctx', 0, 3, \&dataAnalyzeCPUCTX]);
     push(@$array, ['proce', 0, 5, \&dataAnalyzeCPUProc]);
     push(@$array, ['load', 0, 4, \&dataAnalyzeLoad]);
  }
  if ($subsys=~/E/) 
  {
    push(@$array, ['ipmi', 0, 4, \&dataAnalyzeIPMI]);
  }
  if ($subsys=~/d/i) 
  {
    push(@$array, ['disk', 0, 4, \&dataAnalyzeDisk]);
    push(@$array, ['par', 0, 3, \&dataAnalyzePar]);
  }
  if ($subsys=~/i/) 
  {
    push(@$array, ['fs-', 0, 3, \&dataAnalyzeInode]);
  }
  if ($subsys=~/f/i) 
  {
    if ($nfs4CFlag)
    {
      push(@$array, ['nfsc-net', 0, 8, \&dataAnalyzeNFSClient]);
      push(@$array, ['nfsc-rpc', 0, 8, \&dataAnalyzeNFSClientRPC]);
      push(@$array, ['nfsc-proc2', 0, 10, \&dataAnalyzeNFSClientProc2]) if $nfs2Flag;
      push(@$array, ['nfsc-proc3', 0, 10, \&dataAnalyzeNFSClientProc3]) if $nfs3Flag;
      push(@$array, ['nfsc-proc4', 0, 10, \&dataAnalyzeNFSClientProc4]) if $nfs4Flag;
      push(@$array, ['nfs-net', 0, 8, \&dataAnalyzeNFSClient]);
      push(@$array, ['nfs-rpc', 0, 8, \&dataAnalyzeNFSClientRPC]);
      push(@$array, ['nfs-proc2', 0, 10, \&dataAnalyzeNFSClientProc2]) if $nfs2Flag;
      push(@$array, ['nfs-proc3', 0, 10, \&dataAnalyzeNFSClientProc3]) if $nfs3Flag;
      push(@$array, ['nfs-proc4', 0, 10, \&dataAnalyzeNFSClientProc4]) if $nfs4Flag;
    } else {
      push(@$array, ['nfss-net', 0, 8, \&dataAnalyzeNFSServer]);
      push(@$array, ['nfss-rpc', 0, 8, \&dataAnalyzeNFSServerRPC]);
      push(@$array, ['nfss-proc2', 0, 10, \&dataAnalyzeNFSServerProc2]) if $nfs2Flag;
      push(@$array, ['nfss-proc3', 0, 10, \&dataAnalyzeNFSServerProc3]) if $nfs3Flag;
      push(@$array, ['nfss-proc4', 0, 10, \&dataAnalyzeNFSServerProc4]) if $nfs4Flag;
      push(@$array, ['nfs-net', 0, 8, \&dataAnalyzeNFSServer]);
      push(@$array, ['nfs-rpc', 0, 8, \&dataAnalyzeNFSServerRPC]);
      push(@$array, ['nfs-proc2', 0, 10, \&dataAnalyzeNFSServerProc2]) if $nfs2Flag;
      push(@$array, ['nfs-proc3', 0, 10, \&dataAnalyzeNFSServerProc3]) if $nfs3Flag;
      push(@$array, ['nfs-proc4', 0, 10, \&dataAnalyzeNFSServerProc4]) if $nfs4Flag;
    }
  }
  if ($subsys=~/m/)
  {
    if ($kernel2_4) 
    {
      push(@$array, ['page', 0, 4, \&dataAnalyzeMemPage24]);
      push(@$array, ['swap', 0, 4, \&dataAnalyzeMemSwapInOut24]);
      push(@$array, ['Mem:', 0, 4, \&dataAnalyzeMem24]);
      push(@$array, ['Cached', 0, 6, \&dataAnalyzeMemCached24]);
      push(@$array, ['Swap:', 0, 5, \&dataAnalyzeMemSwap24]);
      push(@$array, ['Active', 0, 6, \&dataAnalyzeMemActive24]);
      push(@$array, ['Inact', 0, 5, \&dataAnalyzeMemInactive24]);

    }
    if ($kernel2_6) 
    {
      push(@$array, ['pg', 0, 2, \&dataAnalyzeMemPage26]);
      push(@$array, ['Huge', 0, 4, \&dataAnalyzeMemHuge26]);
      push(@$array, ['Swap', 0, 4, \&dataAnalyzeMemSwap26]);
      push(@$array, ['pswp', 0, 4, \&dataAnalyzeMemPageSwap26]);
      push(@$array, ['Mem', 0, 3, \&dataAnalyzeMem26]);
      push(@$array, ['procs', 0, 5, \&dataAnalyzeMemProcs26]);
      push(@$array, ['Buffers', 0, 7, \&dataAnalyzeMemBuffers26]);
      push(@$array, ['Cached', 0, 6, \&dataAnalyzeMemCached26]);
      push(@$array, ['Dirty', 0, 5, \&dataAnalyzeMemDirty26]);
      push(@$array, ['Active', 0, 6, \&dataAnalyzeMemActive26]);
      push(@$array, ['Inactive', 0, 8, \&dataAnalyzeMemInactive26]);
      push(@$array, ['Anon', 0, 4, \&dataAnalyzeMemAnon26]);
      push(@$array, ['Mapped', 0, 6, \&dataAnalyzeMemMap26]);
      push(@$array, ['Slab:', 0, 5, \&dataAnalyzeMemSlab26]);
      push(@$array, ['Committed_AS:', 0, 13, \&dataAnalyzeMemCommit26]);
      push(@$array, ['SUnreclaim', 0, 10, \&dataAnalyzeMemSUnreclaim26]);
    }
  }
  if ($subsys=~/s/) 
  {
    push(@$array, ['sock', 0, 4, \&dataAnalyzeSocket]);
  }
  if ($subsys=~/n/i)
  {
    push(@$array, ['Net', 0, 3, \&dataAnalyzeNetwork]);
  }
  if ($subsys=~/t/i)
  {
    push(@$array, ['TcpExt', 0, 6, \&dataAnalyzeTCP]);
  }
  if ($subsys=~/x/i)
  {
    push(@$array, ['Elan', 0, 4, \&dataAnalyzeELAN]);
    push(@$array, ['ib', 0, 2, \&dataAnalyzeInfiniband]);
  }

  return $array;
}

sub dataAnalyzeProcesses
{
  my ($type, $data, $procPidNow, $threadFlag) = @_;
...
#  $interval2Print=1;
  ...
  {
    return $procRChar=  int substr($data, 10) if (substr($data, 3, 2) eq "rc");
    return $procWChar=  int substr($data, 10) if (substr($data, 3, 2) eq "wc");
    return $procRBytes= int substr($data, 15) if (substr($data, 3, 2) eq "re");
    return $procWBytes= int substr($data, 16) if (substr($data, 3, 2) eq "wr");
    return $procSyscr=  int substr($data, 10) if (substr($data, 3, 5) eq "syscr");
    return $procSyscw=  int substr($data, 10) if (substr($data, 3, 5) eq "syscw");
  ...
  return unless defined($procSTimeTot[$i]);
  if (substr($data, 0, 2) eq 'Vm')
  {
    my ($procType, $value) = unpack('A6x2A8', $data);
    return $procVmPeak[$i]= int $value if ($procType eq 'VmPeak');
    return $procVmSize[$i]= int $value if ($procType eq 'VmSize');
    return $procVmLck[$i]=  int $value if ($procType eq  'VmLck:');
    return $procVmHWM[$i]=  int $value if ($procType eq  'VmHWM:');
    return $procVmRSS[$i]=  int $value if ($procType eq  'VmRSS:');
    return $procVmData[$i]= int $value if ($procType eq 'VmData');
    return $procVmStk[$i]=  int $value if ($procType eq  'VmStk:');
    return $procVmExe[$i]=  int $value if ($procType eq  'VmExe:');
    return $procVmLib[$i]=  int $value if ($procType eq  'VmLib:');
    return $procVmPTE[$i]=  int $value if ($procType eq  'VmPTE:');

  }
  return $procCmd[$i]=substr($data, 4) if (substr($data, 0, 3) eq "cmd");
  return $procTgid[$i]=substr($data, 7) if (substr($data, 0, 2) eq "Tg");
...
sub dataAnalyzeCPU
{
  ...
  $user = $userNow-$userLast[$cpuIndex];
  $user = fix($user) if ($user < 0);

  $nice = $niceNow-$niceLast[$cpuIndex];
  $nice = fix($nice) if ($nice < 0);

  $sys = $sysNow-$sysLast[$cpuIndex];
  $sys = fix($sys) if ($sys < 0);

  $idle = $idleNow-$idleLast[$cpuIndex];
  $idle = fix($idle) if ($idle < 0);

  $wait = $waitNow-$waitLast[$cpuIndex];
  $wait = fix($wait) if ($wait < 0);

  $irq = $irqNow-$irqLast[$cpuIndex];
  $irq = fix($irq) if ($irq < 0);

  $soft = $softNow-$softLast[$cpuIndex];
  $soft = fix($soft) if ($soft < 0);

  $steal = $stealNow-$stealLast[$cpuIndex];
  $steal = fix($steal) if ($steal < 0);
...
sub dataAnalyzeInterrupts
{
  ...
  ($type, @vals)=split(/\s+/, $data, $cpusEnabled+2);
...
sub dataAnalyzeLustre
{
  my $type=shift;
  my $data=shift;

  chomp $data;
  $index= int substr($type, 4);
...
sub dataAnalyzeLustreBuffer
{
  ...
  ($index, $bufNum) = split (substr($type, 6), ":");
...
sub dataAnalyzeLustreLite
{
  my $type=shift;
  my $data=shift;

#  $fs=$1;
  $fs = int substr($type, 6);
...
sub dataAnalyzeLustreLiteRA
{
  my $type=shift;
  my $data=shift;
  my $i;

  $fs= int substr($type, 9);
  chomp $data;
  $fs=$CltFSMap[$fs]    if $playback ne '';
...
sub dataAnalyzeLustreLiteRPC 
{
  ...
  ($index, $bufNum) = split (substr($type, 10), ":");
...
sub dataAnalyzeLustreLDET
{
  my $type=shift;
  my $data=shift;
  my $i;

  $ost = int substr($type, 6);
  chomp $data;
  ($name, $ops, $value)=(split(/\s+/, $data))[0,1,6];
...
sub dataAnalyzeNFSClient
{
  my $type=shift;
  my $data=shift;

  (undef,undef,undef,$nfsTcpConnNow)=split(/\s+/, $data);
...
sub dataAnalyzeNFSClientRPC
{
  my $type=shift;
  my $data=shift;

  my (@rpcFields)=split(/\s+/, $data);
  if (!defined($rpcFields[2]))
  {
...
sub dataAnalyzeNFSServerRPC
{
  my $type=shift;
  my $data=shift;

  my (@rpcFields)=split(/\s+/, $data);
  if (!defined($rpcFields[4]))
  {
    incomplete("RPC", $lastSecs[$rawPFlag]);
    return;
  }
...
sub dataAnalyzeNFSClientProc2 {
  ...
  $nfs2CSeen=1 if ($nfsValuesNow[6] || $nfsValuesNow[8]);
  return unless ($nfs2CSeen);
  ...
  $nfs2CNull=     $nfs2CValue[0];
  $nfs2CGetattr=  $nfs2CValue[1];
  $nfs2CSetattr=  $nfs2CValue[2];
  $nfs2CRoot=     $nfsC2Value[3];
  $nfs2CLookup=   $nfs2CValue[4];
  $nfs2CReadlink= $nfs2CValue[5];
  $nfs2CRead=     $nfs2CValue[6];
  $nfs2CWrcache=  $nfs2CValue[7];
  $nfs2CWrite=    $nfs2CValue[8];
  $nfs2CCreate=   $nfs2CValue[9];
  $nfs2CRemove=   $nfs2CValue[10];
  $nfs2CRename=   $nfs2CValue[11];
  $nfs2CLink=     $nfs2CValue[12];
  $nfs2CSymlink=  $nfs2CValue[13];
  $nfs2CMkdir=    $nfs2CValue[14];
  $nfs2CRmdir=    $nfs2CValue[15];
  $nfs2CReaddir=  $nfs2CValue[16];
  $nfs2CFsstat=   $nfs2CValue[17];
...
sub dataAnalyzeNFSServerProc2
{
  ...
  $nfs2SSeen=1 if ($nfsValuesNow[6] || $nfsValuesNow[8]);
  return unless ($nfs2SSeen);
  ...
  $nfs2SNull=     $nfs2SValue[0];
  $nfs2SGetattr=  $nfs2SValue[1];
  $nfs2SSetattr=  $nfs2SValue[2];
  $nfs2SRoot=     $nfs2SValue[3];
  $nfs2SLookup=   $nfs2SValue[4];
  $nfs2SReadlink= $nfs2SValue[5];
  $nfs2SRead=     $nfs2SValue[6];
  $nfs2SWrcache=  $nfs2SValue[7];
  $nfs2SWrite=    $nfs2SValue[8];
  $nfs2SCreate=   $nfs2SValue[9];
  $nfs2SRemove=   $nfs2SValue[10];
  $nfs2SRename=   $nfs2SValue[11];
  $nfs2SLink=     $nfs2SValue[12];
  $nfs2SSymlink=  $nfs2SValue[13];
  $nfs2SMkdir=    $nfs2SValue[14];
  $nfs2SRmdir=    $nfs2SValue[15];
  $nfs2SReaddir=  $nfs2SValue[16];
  $nfs2SFsstat=   $nfs2SValue[17];
...
sub dataAnalyzeNFSClientProc3
{
  ...
  $nfs3CSeen=1 if ($nfsValuesNow[6] || $nfsValuesNow[7]);
  return unless ($nfs3CSeen);
  ...
  $nfs3CNull=        $nfs3CValue[0];
  $nfs3CGetattr=     $nfs3CValue[1];
  $nfs3CSetattr=     $nfs3CValue[2];
  $nfs3CLookup=      $nfs3CValue[3];
  $nfs3CAccess=      $nfs3CValue[4];
  $nfs3CReadlink=    $nfs3CValue[5];
  $nfs3CRead=        $nfs3CValue[6];
  $nfs3CWrite=       $nfs3CValue[7];
  $nfs3CCreate=      $nfs3CValue[8];
  $nfs3CMkdir=       $nfs3CValue[9];
  $nfs3CSymlink=     $nfs3CValue[10];
  $nfs3CMknod=       $nfs3CValue[11];
  $nfs3CRemove=      $nfs3CValue[12];
  $nfs3CRmdir=       $nfs3CValue[13];
  $nfs3CRename=      $nfs3CValue[14];
  $nfs3CLink=        $nfs3CValue[15];
  $nfs3CReaddir=     $nfs3CValue[16];
  $nfs3CReaddirplus= $nfs3CValue[17];
  $nfs3CFsstat=      $nfs3CValue[18];
  $nfs3CFsinfo=      $nfs3CValue[19];
  $nfs3CPathconf=    $nfs3CValue[20];
  $nfs3CCommit=      $nfs3CValue[21];
...
sub dataAnalyzeNFSServerProc3
{
  ...
  $nfs3SSeen=1 if ($nfsValuesNow[6] || $nfsValuesNow[7]);
  return unless ($nfs3SSeen);
  ...
  $nfs3SNull=        $nfs3SValue[0];
  $nfs3SGetattr=     $nfs3SValue[1];
  $nfs3SSetattr=     $nfs3SValue[2];
  $nfs3SLookup=      $nfs3SValue[3];
  $nfs3SAccess=      $nfs3SValue[4];
  $nfs3SReadlink=    $nfs3SValue[5];
  $nfs3SRead=        $nfs3SValue[6];
  $nfs3SWrite=       $nfs3SValue[7];
  $nfs3SCreate=      $nfs3SValue[8];
  $nfs3SMkdir=       $nfs3SValue[9];
  $nfs3SSymlink=     $nfs3SValue[10];
  $nfs3SMknod=       $nfs3SValue[11];
  $nfs3SRemove=      $nfs3SValue[12];
  $nfs3SRmdir=       $nfs3SValue[13];
  $nfs3SRename=      $nfs3SValue[14];
  $nfs3SLink=        $nfs3SValue[15];
  $nfs3SReaddir=     $nfs3SValue[16];
  $nfs3SReaddirplus= $nfs3SValue[17];
  $nfs3SFsstat=      $nfs3SValue[18];
  $nfs3SFsinfo=      $nfs3SValue[19];
  $nfs3SPathconf=    $nfs3SValue[20];
  $nfs3SCommit=      $nfs3SValue[21];
...
sub dataAnalyzeNFSClientProc4
{
  ...
  $nfs4CSeen=1 if ($nfsValuesNow[1] || $nfsValuesNow[2]);
  return unless($nfs4CSeen);
  ...
  $nfs4CNull=    $nfs4CValue[0];
  $nfs4CRead=    $nfs4CValue[1];
  $nfs4CWrite=   $nfs4CValue[2];
  $nfs4CCommit=  $nfs4CValue[3];
  $nfs4CSetattr= $nfs4CValue[9];
  $nfs4CFsinfo=  $nfs4CValue[10];
  $nfs4CAccess=  $nfs4CValue[17];
  $nfs4CGetattr= $nfs4CValue[18];
  $nfs4CLookup=  $nfs4CValue[19];
  $nfs4CRemove=  $nfs4CValue[21];
  $nfs4CRename=  $nfs4CValue[22];
  $nfs4CLink=    $nfs4CValue[23];
  $nfs4CSymlink= $nfs4CValue[24];
  $nfs4CCreate=  $nfs4CValue[25];
  $nfs4CPathconf=$nfs4CValue[26];
  $nfs4CReadlink=$nfs4CValue[28];
...
sub dataAnalyzeNFSServerProc4
{
  ...
  $nfs4SSeen=1 if ($nfsValuesNow[25] || $nfsValuesNow[38]);
  return unless ($type=~/^nfs(.?)-proc4ops/ && $nfs4SSeen);
  ...
  $nfs4SAccess=   $nfs4SValue[3];
  $nfs4SCommit=   $nfs4SValue[5];
  $nfs4SCreate=   $nfs4SValue[6];
  $nfs4SGetattr=  $nfs4SValue[9];
  $nfs4SLink=     $nfs4SValue[11];
  $nfs4SLookup=   $nfs4SValue[15];
  $nfs4SRead=     $nfs4SValue[25];
  $nfs4SReaddir=  $nfs4SValue[26];
  $nfs4SReadlink= $nfs4SValue[27];
  $nfs4SRemove=   $nfs4SValue[28]; 
  $nfs4SRename=   $nfs4SValue[29];
  $nfs4SSetattr=  $nfs4SValue[34];
...
sub dataAnalyzeMemActiveInactive24
{
  my $type=shift;
  my $data=shift;
  my $i;
...
sub dataAnalyzeMemProcs26
{
  my ($type, $data) = @_;

  # never include ourselves in count of running processes
  return $procsRun=(split(/\s+/, $data))[0] - 1 if $type=~/^procs_r/;
  return $procsBlock=(split(/\s+/, $data))[0] if $type=~/^procs_b/;
}

sub dataAnalyzeMemPage26
{
  if ($_[0]=~/^pgpgin/)
  {
    $pagein=$_[1] - $pageinLast;
    $pagein=fix($pagein) if ($pagein < 0);
    $pageinLast=$_[1];
  }
  elsif ($_[0]=~/^pgpgout/)
  {
    $pageout=$_[1] - $pageoutLast;
    $pageout=fix($pageout) if ($pageout < 0);
    $pageoutLast=$_[1];
  }
  elsif ($_[0]=~/^pgfault/)
  {
    $pagefault=$_[1] - $pagefaultLast;
    $pagefault=fix($pagefault) if ($pagefault < 0);
    $pagefaultLast=$_[1];
  }
  elsif ($_[0]=~/^pgmaj/)
  {
    $pagemajfault=$_[1] - $pagemajfaultLast;
    $pagemajfault=fix($pagemajfault) if ($pagemajfault < 0);
    $pagemajfaultLast=$_[1];
  }
}

sub dataAnalyzeMemPageSwap26
{
  if ($_[0]=~/^pswpin/)
  {
    $swapin=$_[1] - $swapinLast;
    $swapin=fix($swapin) if ($swapin < 0);
    $swapinLast=$_[1];
  }
  elsif ($_[0]=~/^pswpout/)
  {
    $swapout=$_[1] - $swapoutLast;
    $swapout=fix($swapout) if ($swapout < 0);
    $swapoutLast=$_[1];
  }
}

sub dataAnalyzeMem26
{
  return $memTot= (split(/\s+/, $_[1]))[0] if $_[0]=~/^MemTotal/;
  return $memFree= (split(/\s+/, $_[1]))[0] if $_[0]=~/^MemFree/;
}

sub dataAnalyzeMemBuffers26
{
  $memBuf= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemCached26
{
  $memCached= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemDirty26
{
  $dirty= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemActive26
{
  $active= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemInactive26
{
  $inactive= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemSlab26
{
  $memSlab= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemAnon26
{
  $memAnon= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemMap26
{
  $memMap= (split(/\s+/, $_[1]))[0] + $memAnon;
}

sub dataAnalyzeMemCommit26
{
  $memCommit= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemSUnreclaim26
{
  $memSUnreclaim= (split(/\s+/, $_[1]))[0];
}

sub dataAnalyzeMemHuge26
{
  return $memHugeTot= (split(/\s+/, $_[1]))[0]    if $_[0]=~/^HugePages_T/;
  return $memHugeFree= (split(/\s+/, $_[1]))[0]   if $_[0]=~/^HugePages_F/;
  return $memHugeRsvd= (split(/\s+/, $_[1]))[0]   if $_[0]=~/^HugePages_R/;
}

sub dataAnalyzeMemSwap26
{
  return $swapTotal= (split(/\s+/, $_[1]))[0]  if $_[0]=~/^SwapT/;
  return $swapFree= (split(/\s+/, $_[1]))[0]   if $_[0]=~/^SwapF/;
  return $swapCached= (split(/\s+/, $_[1]))[0] if $_[0]=~/^SwapC/;
}

#    S o c k e t    S t a t s

sub dataAnalyzeSocket
{
  my $type=shift;
  my $data=shift;
...
sub dataAnalyzeNetwork
{
  ...
  $netTxErrs[$netIndex]=$netTxErr[$netIndex]+$netTxDrp[$netIndex]+
    $netTxFifo[$netIndex]+$netTxColl[$netIndex]+$netTxCar[$netIndex];
...
sub printPlotHeaders
{
  ...
  writeDataFinalSocket('', \&headersAll)
        if ($sockFlag && scalar(@sockets) && $export eq '');
...
sub printPlot
{
  ...
  print "$datetime$oneline\n" if (!$logToFileFlag && !$sockFlag);
  writeDataFinalSocket($datetime, \&oneline) 
        if ($sockFlag && scalar(@sockets) && $export eq '');
...
sub printPlotProc
{
  ...
  if (!$headersPrintedProc)
  {
    ...
    print "$procHeaders\n" if (!$logToFileFlag && !$sockFlag);
    AppendDataToLog('', \$procHeaders, PRC, $ZPRC, 'proc') if ($logToFileFlag);
    writeDataFinalSocket('', \&procHeaeders)
        if ($sockFlag && scalar(@sockets) && $export eq '');
  ...
  foreach $pid (sort keys %procIndexes)
  {
    ...
    # Do some tests before calling cvt to reduce call overhead
    unless ($wideFlag) {
      $majFlt = int($majFlt+.5) if $majFlt>0;
      $majFlt = cvt($majFlt) unless ($majFlt==0);

      $minFlt = int($minFlt+.5) if $minFlt>0;
      $minFlt = cvt($minFlt) unless ($minFlt==0);
    }

   # Username comes from translation hash OR we just print the UID
    my $procPlot = sprintf("%s${SEP}%d${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%s${SEP}%d.%02d${SEP}%d.%02d${SEP}%d${SEP}%d:%02d.%02d${SEP}%d${SEP}%d${SEP}%d${SEP}%d${SEP}%d${SEP}%d${SEP}%d${SEP}%s${SEP}%s${SEP}%s",
          ...
          # this replaces the call to cvtT1
          int $procSTime[$i]/$HZ,
          $procSTime[$i] % $HZ,
          ...
          # This replaces the call to cvtT2 
          int ($procSTimeTot[$i]+$procUTimeTot[$i])/($HZ*60),
          int (($procSTimeTot[$i]+$procUTimeTot[$i])/$HZ) % 60,
          ($procSTimeTot[$i]+$procUTimeTot[$i]) % $HZ,

          $procRKB[$i]/$interval2Secs,
          $procWKB[$i]/$interval2Secs,
          $procRKBC[$i]/$interval2Secs,
          $procWKBC[$i]/$interval2Secs,
          $procRSys[$i]/$interval2Secs,
          $procWSys[$i]/$interval2Secs,
          $procCKB[$i]/$interval2Secs,
          $majFlt, $minFlt,
    ...
    AppendDataToLog('', \$procPlot, PRC, $ZPRC, 'proc') if ($logToFileFlag);
    writeDataFinalSocket('', \&procPlot)
      if ($sockFlag && scalar(@sockets) && $export eq '');
  }
}
...
sub printPlotSlab
{
  ...
  if ($slabPlot ne '')
  {
    ...
    {
      AppendDataToLog('', \$slabPlot, SLB, $ZSLB, 'slb') if ($logToFileFlag);
      writeDataFinalSocket('', \&slabPlot)
          if ($sockFlag && scalar(@sockets) && $export eq '');
    }
